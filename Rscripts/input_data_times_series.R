...
set_fast_options() # Optional, but should speed up
...
for(i in list.dirs(data_dir, recursive=FALSE)){
  if(file.exists(paste0(i,'/','stack.vrt'))){
    ...
    # Try using NETCDF -------- not implemented yet
    ...
    log_filename <- file.path(i,'/', paste0(format(Sys.time(), "%Y-%m-%d-%H-%M-%S"), "_gdalwarp",  ".log"))
    ...
    if(substr(dates[1],1,4)<=historical_year_beg &
      ...
      if(mask_data==1){
        ...
        stack_outputfile <- paste0(strsplit(i,'/','.tif'),'_masked.tif')
        ...
        rm(resamp,r.map1,r.map1.xmin,r.map1.ymin,r.map1.xmax,r.map1.ymax,r.map1.xres,r.map1.yres,r.map1.proj,forestmask,r.map2,
           r.map2.xmin,r.map2.ymin,r.map2.xmax,r.map2.ymax,r.map2.xres,r.map2.yres,proj_match)
